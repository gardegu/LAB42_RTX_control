...
/* implemented in gnuchess.c */
void EnPassant (short int xside, short int f, short int t, short int iop);
...
void LoadGame(FILE *fd, char fname[256])
{
  int c, i, j;
  char *p;
  short sq;
 
  ShowMessage ("warning: loading file without header.");
  NewGame();
...
GetGame (void)
{
  ...
  if ((fd = fopen (fname, "r")) != NULL)
	...
	TCflag = true;
      ...
      ShowMessage ("warning: can't find file.");
  }
  /* arnoud@science.uva.nl says: shouldn't the following calls not be protected? */
  InitializeStats ();
  UpdateDisplay (0, 0, 1, 0);
  Sdepth = 0;
}
...
SaveGame (void)
{
  ...
  if ((fd = fopen (fname, "w")) != NULL)
	...
	w = "computer";
      ...
      fprintf (fd, "White Clock %ld Moves %d\nBlack Clock %ld Moves %d\n\n",
